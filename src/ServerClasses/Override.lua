...
cs.class 'Override' (function (this)
	...
	function this:init (model, network)
		...
		self.propagator.eventFired:Connect(function (player, object)
			if Util.playerAlive(player) and Access.IsPrivilegedUser(player) then
				self:resetNetwork()
			end
		end)
	...
	function this.member:pollPlayerMovements()
		while true do
			...
			for i,v in next, game.Players:GetPlayers() do
				if Util.playerAlive(v) and Access.IsPrivilegedUser(v) then
					local mainPart = Util.playerCharacterMainPart(v.Character)
					if (mainPart.Position - self.model.Button.Position).magnitude < self.maxDistanceFromButton then
						playerNear = true
						break
					end
				end
